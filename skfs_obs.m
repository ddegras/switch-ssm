function [Mf,Ms,xf,xs,x0,P0,L,sum_CP,sum_MP,sum_Ms2,sum_Mxy,sum_P,sum_Pb] = ...
    skfs_obs(y,M,p,A,C,Q,R,mu,Sigma,Pi,Z,beta,safe,abstol,reltol,verbose)
...
for j = 1:M
    idx1 = (j-1)*p*r+1:j*p*r;
    idx2 = (j-1)*r+1:j*r;
    Atmp(idx1,idx1) = A(:,:,j);     % for filtering 
    Asmall(idx2,idx1) = A(1:r,:,j); % for smoothing
    Ctmp(:,idx1,j) = C(:,:,j); 
    Qtmp(idx1,idx1) = Q(:,:,j);
    Stmp(idx1,idx1) = Sigma(:,:,j);
end
mu = mu(:);
A = Atmp;
C = Ctmp;
...
for j=1:M
    ...
    Ve = 0.5 * (Ve+Ve.');    
    xf1(:,j,1) = mu + Sigma * C_j.' * (Ve\e);
    Vf1t(:,:,j) = Sigma - Sigma * C_j.' * (Ve\C_j) * Sigma;
